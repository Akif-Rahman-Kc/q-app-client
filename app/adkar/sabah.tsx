...
export default function SabahScreen() {
    const router = useRouter();
    const [counts, setCounts] = useState<Record<number, number>>(() => {
        const init: Record<number, number> = {};
        sabahAdkar.forEach(a => { init[a.id] = a.repeat; });
        return init;
    });
    ...
    useFocusEffect(
        React.useCallback(() => {
            const loadData = async () => {
                ...
                const categoryFavs = favs.filter(f => f.category === 'sabah').map(f => f.id);
                setFavorites(new Set(categoryFavs));

                const init: Record<number, number> = {};
                sabahAdkar.forEach(a => { init[a.id] = a.repeat; });
                const dailyCounts = await getDailyAdkarCounts('sabah', init);
    ...
    const handleToggleFavorite = async (id: number) => {
        const isFav = await toggleAdkarFavorite('sabah', id);
        setFavorites(prev => {
    ...
    const completed = useMemo(
        () => sabahAdkar.filter(a => counts[a.id] === 0).length,
        [counts]
    );
    const total = sabahAdkar.length;
    ...
    const handleCount = useCallback((id: number) => {
        setCounts(prev => {
            ...
            const newlyCompleted = sabahAdkar.filter(a => newCounts[a.id] === 0).length;
            const isFullyCompleted = newlyCompleted === total;

            saveDailyAdkarProgress('sabah', newCounts, isFullyCompleted).catch(console.error);
        ...
        <LinearGradient colors={['#050f05', '#0a1a0f', '#0d2210']} style={{ flex: 1 }}>
            ...
            <View style={{ paddingTop: 40, paddingHorizontal: 20, paddingBottom: 16 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
                    ...
                    <View style={{ flex: 1 }}>
                        <Text style={{ color: '#10b981', fontSize: 11, fontWeight: '800', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 2 }}>
                            MORNING
                        </Text>
                ...
                {/* Progress Bar */}
                <View style={{ backgroundColor: '#1a2e1a', borderRadius: 12, overflow: 'hidden', height: 8, marginBottom: 8 }}>
                    <View style={{
                ...
                {sabahAdkar.map((adkar, index) => {
                    ...
                    return (
                        <View
                            key={adkar.id}
                            style={{
                                backgroundColor: isDone ? '#0a1f0a' : '#0d1a0d',
                                borderRadius: 24,
                                marginBottom: 16,
                                borderWidth: 1,
                                borderColor: isDone ? '#10b98150' : '#1a2e1a',
                                shadowColor: '#000',
                        ...
                        >
                            ...
                            <View style={{ paddingHorizontal: 18, paddingBottom: 16, borderTopWidth: 1, borderTopColor: '#1a2e1a', paddingTop: 14 }}>
                                ...
                                <Text style={{ color: '#a7f3d0', fontSize: 14, fontStyle: 'italic', marginBottom: 12, lineHeight: 22 }}>
                                ...
                                {!!(adkar.en_benefit || adkar.benefit) && (
                                    <View style={{ backgroundColor: '#051205', borderRadius: 12, padding: 14, borderLeftWidth: 4, borderLeftColor: '#10b981' }}>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 16, paddingTop: 6 }}>
                                ...
                                <Pressable
                                    ...
                                    className={`w-10 h-10 rounded-full items-center justify-center border-2 border-[#10b981] ${isDone ? 'bg-[#10b981]' : 'bg-transparent'} active:scale-95 active:opacity-80`}
                                >
                                    <Check
                                        size={20}
                                        color={isDone ? '#ffffff' : '#10b981'}
                                        strokeWidth={5}
                                    />
                                </Pressable>
                            </View>
